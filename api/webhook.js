...
async function handleFind(userId, chatId, env, api, fromInternal) {
  ...
  // (anti race condition + anti stale data dari testing/versi lama)
  const partnerUser = await acGetUser(env, partnerId);
  if (!partnerUser || !partnerUser.gender || partnerUser.status !== "searching") {
    // Partner stale atau sudah di-claim — hapus dari queue dan coba lagi
    await acRemoveFromQueue(env, partnerId);

    // Coba cari partner lain dari queue yang sudah dibersihkan
    const nextPartnerId = await acPickPartner(env, userId);
    if (!nextPartnerId) {
      return api.send({
        chat_id: chatId,
        text:
    ...
    // Verifikasi partner kedua juga
    const nextPartnerUser = await acGetUser(env, nextPartnerId);
    if (!nextPartnerUser || !nextPartnerUser.gender || nextPartnerUser.status !== "searching") {
      await acRemoveFromQueue(env, nextPartnerId);
      return api.send({
        chat_id: chatId,
        text:
          "🔍 *Nyariin partner buat kamu...*\n\n" +
    ...
    // Match dengan partner kedua
    await acRemoveFromQueue(env, userId);
    await acRemoveFromQueue(env, nextPartnerId);
    await acSetUser(env, userId,         { ...user,            status: "chatting" });
    await acSetUser(env, nextPartnerId,  { ...nextPartnerUser, status: "chatting" });
    await acSetSession(env, userId, nextPartnerId);
    ...
    await api.send({ chat_id: Number(nextPartnerId), text: connMsg });
    return;
  }
...
async function handleAdminCmd(text, chatId, env, api) {
  ...
  if (cmd === ".flushqueue") {
    // Hapus semua antrian pencarian — berguna kalau ada stale data dari versi lama
