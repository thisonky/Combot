...
import {
  ...
  dbGetAdminReply, dbSetAdminReply, dbDelAdminReply, syncToSpreadsheet
} from "./_db.js";
...
export default async function handler(req, res) {
  ...
  if (req.method !== "POST") {
    return res.status(200).send(JSON.stringify({ status: "Hardened Production Engine Active" }));
  }
  ...
  try {
    ...
    console.error("[Fatal Webhook Root Guard]:", err.message);
    return res.status(200).send(JSON.stringify({ ok: true }));
  }
}
...
async function handleMessage(msg, env, api) {
  ...
  if (await dbIsBlocked(env, uidNum)) {
    return await api.send({ chat_id: chatId, text: "❌ Kamu telah diblokir dari bot ini karena melanggar ketentuan." });
  }

  // === AMANKAN LOGIKA UTUH PANEL ADMIN & INSTRUKSI FITUR COMMAND ===
  if (uidNum === env.ADMIN_ID) {
    if (text === "/panel" || text === "/status") {
      ...
      const panelTxt = `⚙️ *PANEL KONTROL UTAMA ADMIN* (Full Operational)\n━━━━━━━━━━━━━━━\n` +
                       `👤 Total User Registrasi: *${uCount}*\n` +
                       `🔒 Pengguna Diblokir: *${bCount}*\n` +
                       `🔇 Pengguna Dimute: *${mCount}*\n` +
                       `🚫 Kata Sensor Aktif: *${kCount}*\n\n` +
                       `👑 *MANAJEMEN FITUR & BISNIS (PROFIT)*:\n` +
                       `• \`/addprem [UID] [3d/1w/1m]\`\n  _Fungsi: Mengubah user biasa menjadi Premium. User bisa menggunakan filter gender saat mencari partner dan mendapatkan link profil rahasia secara otomatis._\n` +
                       `• \`/addad [UID] [jumlah]\`\n  _Fungsi: Memberikan tambahan bonus kuota kupon postingan menfess bersistem Auto-Delete (Terhapus otomatis dari channel dalam x menit) bagi user target._\n\n` +
                       `🛠️ *MANAJEMEN MODERASI & ANTRIAN CHAT*:\n` +
                       `• \`/bc [pesan]\` - Kirim pesan massal/pengumuman global ke seluruh pengguna.\n` +
                       `• \`/block [UID]\` - Melarang pengguna masuk atau berinteraksi dengan bot.\n` +
                       `• \`/unblock [UID]\` - Membuka akses pengguna yang sempat diblokir.\n` +
                       `• \`/mute [UID]\` - Menghambat hak kirim chat/menfess user (tetap bisa buka menu).\n` +
                       `• \`/unmute [UID]\` - Memulihkan status pembatasan bisu pengguna.\n\n` +
                       `🚫 *MANAJEMEN BLACKLIST KEYWORDS*:\n` +
                       `• \`/addkw [kata]\` - Menambahkan sensor kata buruk/kasar otomatis.\n` +
                       `• \`/delkw [kata]\` - Menghapus kata dari daftar sensor sistem.\n` +
                       `• \`/listkw\` - Memeriksa kompilasi daftar kata yang sedang disensor.\n\n` +
                       `🚨 *DARURAT DEVELOPER*:\n` +
                       `• \`/resetredis\` - *WIPE OUT TOTAL* data sesi obrolan aktif dan antrean antrean yang nyangkut.`;
    ...
    if (text === "/resetredis") {
      ...
      if (clearJson.result === "OK") {
        return await api.send({ chat_id: chatId, text: "🔄 *DATABASE UPSTASH REDIS BERHASIL DI-RESET!*", parse_mode: "Markdown" });
      }
      return await api.send({ chat_id: chatId, text: "❌ Gagal mereset database Redis." });
    }
    ...
    if (text.startsWith("/addprem ")) {
      ...
      if (!targetUid || !durationStr) return await api.send({ chat_id: chatId, text: "⚠️ Format: \`/addprem [UID] [3d/1w/1m]\`" });

      let days = durationStr === "3d" ? 3 : durationStr === "1w" ? 7 : durationStr === "1m" ? 30 : 0;
      if (days === 0) return await api.send({ chat_id: chatId, text: "⚠️ Durasi salah! Gunakan: 3d, 1w, atau 1m" });
      ...
      // Sinkronisasikan status premium ke Google Spreadsheet
      await syncToSpreadsheet(env, { action: "add_premium", uid: Number(targetUid), duration: durationStr, expireAt });

      await api.send({ chat_id: chatId, text: `✅ Berhasil menambahkan Premium ke \`${targetUid}\` selama (${days} hari). Data tersinkronisasi ke Spreadsheet.`, parse_mode: "Markdown" });
      return await tgRaw(env.BOT_TOKEN, "sendMessage", {
        chat_id: Number(targetUid),
        text: `🎉 *Pembayaran Premium Dikonfirmasi!*\n━━━━━━━━━━━━━━━\nAkun Anda telah diubah menjadi *User Premium* selama *${days} hari*.\nSekarang kamu bebas memilih kriteria gender partner di menu *Cari Partner* dan mendapatkan keunggulan lainnya!`,
        parse_mode: "Markdown"
      });
    }
    ...
    if (text.startsWith("/addad ")) {
      ...
      if (!targetUid || isNaN(quotaNum)) return await api.send({ chat_id: chatId, text: "⚠️ Format: \`/addad [UID] [Jumlah]\`" });
    ...
    if (text.startsWith("/bc ")) {
      ...
      await api.send({ chat_id: chatId, text: `⏳ Mengirim broadcast ke ${allUsers.length} user...` });
      for (const u of allUsers) {
        await tgRaw(env.BOT_TOKEN, "sendMessage", { chat_id: Number(u), text: `📢 *INFORMASI ADMIN*:\n\n${bcMsg}`, parse_mode: "Markdown" });
      }
      return await api.send({ chat_id: chatId, text: "✅ Broadcast selesai." });
    }

    if (text.startsWith("/block ")) {
      const tUid = text.split(" ")[1];
      if (tUid) { await dbBlock(env, tUid); return await api.send({ chat_id: chatId, text: `✅ User \`${tUid}\` berhasil diblokir.`, parse_mode: "Markdown" }); }
    }
    if (text.startsWith("/unblock ")) {
      const tUid = text.split(" ")[1];
      if (tUid) { await dbUnblock(env, tUid); return await api.send({ chat_id: chatId, text: `✅ Blokir User \`${tUid}\` berhasil dibuka.`, parse_mode: "Markdown" }); }
    }
    if (text.startsWith("/mute ")) {
      const tUid = text.split(" ")[1];
      if (tUid) { await dbMute(env, tUid); return await api.send({ chat_id: chatId, text: `✅ User \`${tUid}\` berhasil di-mute.`, parse_mode: "Markdown" }); }
    }
    if (text.startsWith("/unmute ")) {
      const tUid = text.split(" ")[1];
      if (tUid) { await dbUnmute(env, tUid); return await api.send({ chat_id: chatId, text: `✅ Status mute User \`${tUid}\` berhasil dilepas.`, parse_mode: "Markdown" }); }
    }
    if (text.startsWith("/addkw ")) {
      const kw = text.replace("/addkw ", "");
      if (kw) { await dbAddKw(env, kw); return await api.send({ chat_id: chatId, text: `✅ Kata \`${kw}\` berhasil ditambahkan ke blacklist.`, parse_mode: "Markdown" }); }
    }
    if (text.startsWith("/delkw ")) {
      const kw = text.replace("/delkw ", "");
      if (kw) { await dbDelKw(env, kw); return await api.send({ chat_id: chatId, text: `✅ Kata \`${kw}\` berhasil dihapus dari blacklist.`, parse_mode: "Markdown" }); }
    }
    if (text === "/listkw") {
      const list = await dbListKw(env);
      if (list.length === 0) return await api.send({ chat_id: chatId, text: "🚫 Tidak ada kata terlarang yang disimpan." });
      return await api.send({ chat_id: chatId, text: `🚫 *Daftar Kata Sensor:*\n\n` + list.map(k => `- \`${k}\``).join("\n"), parse_mode: "Markdown" });
    }

    if (text === "/cancelreply") { await dbDelAdminReply(env, env.ADMIN_ID); return await api.send({ chat_id: chatId, text: "✅ Mode balas dibatalkan." }); }
  ...
  if (await dbIsMuted(env, uidNum)) {
    if (text === "🔍 Cari Partner" || text === "📝 Kirim Menfess" || !text.startsWith("/")) {
      return await api.send({ chat_id: chatId, text: "🔇 Kamu sedang di-mute." });
    }
  }
  ...
  if (text.startsWith("/start")) {
    ...
    if (parts.length > 1 && parts[1].startsWith("ref_")) {
      ...
      if (refId !== uidNum && !(await dbHasUsedReferral(env, uidNum))) {
        ...
        await tgRaw(env.BOT_TOKEN, "sendMessage", { chat_id: refId, text: "🎉 Bonus +1 Slot Auto-Delete masuk karena undangan referral!" });
      }
    }
    ...
    return await api.send({ chat_id: chatId, text: "🤖 Menu utama aktif. Silakan pilih navigasi di bawah ini:", reply_markup: getMainMenuKeyboard() });
  }

  // === AMANKAN FITUR FILTER PENCARIAN GENDER PREMIUM ===
  if (text === "🔍 Cari Partner") {
    const session = await acGetSession(env, uidNum);
    if (session) return await api.send({ chat_id: chatId, text: "⚠️ Sesi mengobrol Anda masih aktif!", reply_markup: getChattingKeyboard() });

    const me = await acGetUser(env, uidNum) || {};
    if (!me.gender) return await api.send({ chat_id: chatId, text: "⚠️ Selesaikan registrasi dengan mengetik /start terlebih dahulu." });
  ...
  if (text === "⏭️ Next" || text === "/next") {
    const session = await acGetSession(env, uidNum);
    if (!session) return await api.send({ chat_id: chatId, text: "Anda tidak sedang dalam obrolan." });
    if (await acIsDone(env, uidNum)) return await api.send({ chat_id: chatId, text: "⏳ Tunggu sebentar sebelum berpindah." });

    await acMarkDone(env, uidNum);
    await acDelSession(env, uidNum); await acDelSession(env, session);
    await tgRaw(env.BOT_TOKEN, "sendMessage", { chat_id: session, text: "🛑 Partner mengakhiri obrolan.", reply_markup: getMainMenuKeyboard() });
  ...
  if (text === "🛑 Stop" || text === "/stop") {
    ...
    if (!session) return await api.send({ chat_id: chatId, text: "Anda tidak dalam obrolan.", reply_markup: getMainMenuKeyboard() });

    await acDelSession(env, uidNum); await acDelSession(env, session);
    await tgRaw(env.BOT_TOKEN, "sendMessage", { chat_id: session, text: "🛑 Partner meninggalkan obrolan.", reply_markup: getMainMenuKeyboard() });
    return await api.send({ chat_id: chatId, text: "🛑 Keluar dari sesi obrolan.", reply_markup: getMainMenuKeyboard() });
  }

  const pending = await dbGetPending(env, uidNum);
  if (pending && pending.step === "waiting_text") {
    if (text === "🛑 Batal") { await dbDeletePending(env, uidNum); return await api.send({ chat_id: chatId, text: "❌ Dibatalkan.", reply_markup: getMainMenuKeyboard() }); }
    
    const rawTxt = msg.text || msg.caption || "";
    if (rawTxt && (await dbContainsBlacklistedKw(env, rawTxt))) return await api.send({ chat_id: chatId, text: "❌ Mengandung kata terlarang!" });
  ...
  if (text === "📝 Kirim Menfess") {
    ...
    return await api.send({ chat_id: chatId, text: "📝 Silakan ketik menfess Anda (Teks/Media):", reply_markup: { keyboard: [[{ text: "🛑 Batal" }]], resize_keyboard: true } });
  }

  // === AMANKAN PESAN PROMOSI BERKALA & DESKRIPSI UTUH BENEFIT PREMIUM ===
  if (text === "🎁 Bonus & Referral") {
    ...
    const refTxt = `🎁 *MENU BONUS & PREMIUM USER*\n━━━━━━━━━━━━━━━\n` +
                   `👤 Total Rekomendasi Anda: *${c} User*\n` +
                   `⏱️ Sisa Slot Auto-Delete Anda: *${b} Post*\n\n` +
                   `👑 *KEUNTUNGAN MENJADI AKUN PREMIUM*:\n` +
                   `1. *Filter Kriteria Gender* — Bebas pilih mencari khusus Cowok / Cewek.\n` +
                   `2. *Premium Instant Link* — Membuka tautan profil partner langsung tanpa perlu username telegram.\n` +
                   `3. *Prioritas Antrean Utama* — Masuk ke dalam antrean paling atas (*Matchmaking Priority*) sehingga mempercepat pencarian partner.\n\n` +
                   `👇 Ketuk tombol di bawah untuk info pendaftaran akun premium atau bagikan link referral Anda untuk mendapatkan kuota tambahan secara gratis:`;
  ...
  if (text === "📬 Hubungi Admin") {
    ...
    return await api.send({ chat_id: chatId, text: "📬 Silakan ketik pesan saran/keluhan Anda. Tekan tombol di bawah untuk keluar.", reply_markup: { keyboard: [[{ text: "🛑 Keluar Hubungi Admin" }]], resize_keyboard: true } });
  }

  // === AMANKAN DAN KEMBALIKAN FITUR /DONATE DENGAN MEDIA DUKUNGAN ===
  ...
  if (text === "🚨 Report Partner" || text === "/report") {
    ...
    if (!activeSession) return await api.send({ chat_id: chatId, text: "⚠️ Anda tidak sedang dalam obrolan aktif untuk melaporkan partner." });
    ...
    return await api.send({ chat_id: chatId, text: "✅ Laporan Anda telah terkirim ke Admin." });
  }
  ...
  return await api.send({ chat_id: chatId, text: "Menu Utama:", reply_markup: getMainMenuKeyboard() });
}
...
async function handleCallback(query, env, api) {
  ...
  if (data === "premium_info_buy") {
    ...
    return await api.send({ chat_id: chatId, text: buyTxt, parse_mode: "Markdown" });
  }
  ...
  if (data.startsWith("reg_gender_")) {
    ...
    // Mengeksekusi simpan ke database redis + sync mutlak ke spreadsheet google
    await dbRegisterUser(env, uidNum, pickedGender, query.from.username || "no_username");

    const finishTxt = `✅ Berhasil mendaftar sebagai *${pickedGender === 'male' ? 'Laki-laki 🙋‍♂️' : 'Perempuan 🙋‍♀️'}*.\n━━━━━━━━━━━━━━━\n🌟 Kunjungi menu *🎁 Bonus & Referral* jika ingin upgrade kriteria pencarian Premium.`;
    return await api.edit({ chat_id: chatId, message_id: msgId, text: finishTxt, parse_mode: "Markdown" }).then(async () => {
       await tgRaw(env.BOT_TOKEN, "sendMessage", { chat_id: chatId, text: "🤖 Menu utama aktif:", reply_markup: getMainMenuKeyboard() });
    });
  }
  ...
  if (data.startsWith("match_filter_")) {
    ...
    await acSetUser(env, uidNum, me);
    await api.edit({ chat_id: chatId, message_id: msgId, text: `⚡ _Mencari partner berdasarkan kriteria..._` });
    await acAddToQueue(env, uidNum);
    await triggerMatching(env, uidNum, api);
    return;
  }
  ...
  if (data.startsWith("mf_send_")) {
    ...
    const pending = await dbGetPending(env, uidNum);
    if (!pending) return await api.answer(query.id, "Kedaluwarsa.", true);

    if (action === "cancel") { await dbDeletePending(env, uidNum); return await api.edit({ chat_id: chatId, message_id: msgId, text: "❌ Dibatalkan." }); }
    const autoDelete = action === "autodel";
    if (autoDelete) {
      const s = await dbGetReferralBonus(env, uidNum);
      if (s <= 0) return await api.answer(query.id, "Slot habis!", true);
    ...
    await submitMenfessToChannel(uidNum, pending, autoDelete, env, api);
    return;
  }

  if (data.startsWith("adm_reply_")) { await dbSetAdminReply(env, env.ADMIN_ID, data.replace("adm_reply_", "")); await api.answer(query.id); return await api.send({ chat_id: chatId, text: "Ketik balasan ke pengguna:" }); }
...
async function triggerMatching(env, uidNum, api) {
  ...
  // PREMIUM GENDER CROSS-CHECK INTEGRATION MATCHMAKING
  ...
  if (matchedPartnerId) {
    ...
    await tgRaw(env.BOT_TOKEN, "sendMessage", { chat_id: Number(uidNum), text: txtMe, parse_mode: "Markdown", reply_markup: getChattingKeyboard() });
    await tgRaw(env.BOT_TOKEN, "sendMessage", { chat_id: matchedPartnerId, text: txtPartner, parse_mode: "Markdown", reply_markup: getChattingKeyboard() });
  }
}
